...
public class ScottController {
	...
	public String empdept(HttpServletRequest request , @RequestParam(value = "deptno", defaultValue = "10") int deptno) {
		
		logger.info("> ScottController.empdept() 컨트롤러 메서드 호출됨...");
		
		ArrayList<DeptDTO> deptlist = this.deptMapper.selectDept();
		
		ArrayList<EmpdeptDTO> list = this.empdeptMapper.selectEmpdept(deptno);
		request.setAttribute("list", list);
